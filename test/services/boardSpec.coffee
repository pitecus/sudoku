describe 'Board Services', ->
  ...
  describe 'Starting the board', ->
    ...
    it 'should notify of event change', ->
      # Setup the callback.
      cbCount = 0
      cb = (event, data) ->
        cbCount += 1
        assert.strictEqual data[0].x, 1
        assert.strictEqual data[0].y, 1
        assert.deepEqual data[0].values, [1, 2, 3, 4, 5, 6, 7, 8, 9]
        assert.strictEqual data[1].x, 1
        assert.strictEqual data[1].y, 1
        assert.deepEqual data[1].values, [3]
      board.addListener 'change', cb
      board.setValues 1, 1, [3]
      assert.strictEqual cbCount, 1

    it 'should notify of event selection', ->
      # Setup the callback.
      cbCount = 0
      cb = (event, data) ->
        cbCount += 1
